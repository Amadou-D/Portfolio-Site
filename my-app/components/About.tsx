...
const About: React.FC<AboutProps> = ({ onClose }) => {
  ...
  return (
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 w-full h-full flex items-center justify-center z-[100] p-2 sm:p-4"
    >
      ...
      <motion.div 
        ...
        className="relative z-20 w-full max-h-[90vh] md:max-h-[85vh] max-w-5xl mx-auto bg-white/80 backdrop-blur-lg rounded-xl shadow-2xl overflow-hidden flex flex-col"
      >
        {/* Header with photo and name - Fixed positioning for close button */}
        ...
        {/* Tabs navigation - Made horizontally scrollable */}
          ...
          <div className="flex min-w-max">
            {tabItems.map((tab) => (
              <button
                ...
                className={`px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm md:text-base font-medium whitespace-nowrap transition-colors duration-200 relative
                  ${activeTab === tab.id 
        ...
        {/* Content area - Improved scrolling for mobile */}
